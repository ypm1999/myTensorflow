...
//extern "C" void cpy_to_A(float* A, int n);
//extern "C" void cpy_to_B(float* B, int n);
extern "C" void matmul_gpu(float* A, float* B, float* C, int n, int m, int k, float beta);

void matmul_g(float* a, float* b, float* c, uint n, uint k, uint m, float beta = 0){

    matmul_gpu(a, b, c, n, m, k, beta);
...
const uint memory_size = (50 * 28 * 28 * 5 * 5 * 32);
...
void run_conv2d(uint L, uint R, const int &new_n, const int &new_m, const int &u, const int &d,
    ...
    if(L == 0)
        th.join();
    matmul_g(image, flt, res, (uint)((R - L) * new_n * new_m), fn * fm * last, out);
}
...
void conv2d(float *_img, float *_flt, float *_result,
    ...
    th = std::thread(set_zero,batch_size, new_n, new_m, l, r, u, d, fin);
//    cpy_to_B(flt, fn * fm * fin * fout);
    while(now < num){
...
void backup_conv2d_image(float *_img, float *_flt, float *_result,
    ...
    th = std::thread(set_zero, batch_size, new_n, new_m, l, r, u, d, fout);
//    cpy_to_B(flt, fn * fm * fin * fout);
    while(now < num){
        uint nex =  now + batch_size;
        if(nex > num)
...
//    for(int i = 0; i < n; i++)
//        for(int j = 0; j < k; j++)
//            A[i * k + j] = rand() % 100000000 / 100000000.;
//    for(int i = 0; i < k; i++)
//        for(int j = 0; j < m; j++)
//            B[i * m + j] = rand() % 100000000 / 100000000.;
//    for(int i = 0; i < n; i++)
//        for(int j = 0; j < m; j++)
//            C[i * m + j] = D[i * m + j] = rand() % 100000000 / 100000000.;
//    matmul_gpu(A, B, C, n, m, k, 0);
//    out(C, n, m);
//    puts("");
//    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, n, m, k, (float)1, A, k, B, m, 0, D, m);
//    out(D, n, m);
//    for(int i = 0; i < n; i++)
//        for(int j = 0; j < m; j++)
//            if(fabs(C[i * m + j] - D[i * m + j]) > 1e-4){
//        printf("%d %d\n", i, j);
//    }
//
//    return 0;
